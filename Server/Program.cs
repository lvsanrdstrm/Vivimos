using Microsoft.AspNetCore.Identity.Data;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Http;
using System.IO;
using System.Threading.Tasks;
using Newtonsoft.Json;

var builder = WebApplication.CreateBuilder(args);
var app = builder.Build();

app.MapPost("/auth/login", async (HttpContext context) =>
{
    var request = context.Request;
    var requestBody = await new StreamReader(request.Body).ReadToEndAsync();
    var loginRequest = JsonConvert.DeserializeObject<LoginRequest>(requestBody);

    // Implement authentication logic using loginRequest.Username and loginRequest.Password

    // Example authentication logic:
    if (loginRequest.Username == "rosa.parks" && loginRequest.Password == "bus")
    {
        // Authentication successful
        Console.WriteLine(loginRequest.Username + loginRequest.Password);
        return Results.Ok("Authentication successful");
    }
    else
    {
        // Authentication failed
        return Results.BadRequest("Authentication failed");
    }

});

//app.MapPost("/auth/login", () => @"
//[
//    {
//      ""id"": ""1"",
//      ""username"": ""rosa_parks"",
//      ""email"": ""rosa.parks@example.com"",
//      ""password"": ""bus"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""2"",
//      ""username"": ""malala_yousafzai"",
//      ""email"": ""malala.yousafzai@example.com"",
//      ""password"": ""nobelpeaceprize"",
//      ""role"": ""admin""
//    },
//    {
//      ""id"": ""3"",
//      ""username"": ""emma_watson"",
//      ""email"": ""emma.watson@example.com"",
//      ""password"": ""heforshe"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""4"",
//      ""username"": ""bell_hooks"",
//      ""email"": ""bell.hooks@example.com"",
//      ""password"": ""feministtheory"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""5"",
//      ""username"": ""sojourner_truth"",
//      ""email"": ""sojourner.truth@example.com"",
//      ""password"": ""aintiawoman"",
//      ""role"": ""admin""
//    },
//    {
//      ""id"": ""63b0"",
//      ""username"": ""aagrg"",
//      ""email"": ""erik.cronqvist@gmail.com"",
//      ""password"": ""aefaef""
//    },
//    {
//      ""id"": ""8dde"",
//      ""username"": ""aaga"",
//      ""email"": ""erik.cronqvist@gmail.com"",
//      ""password"": ""gagaga"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""e712"",
//      ""username"": ""bauta"",
//      ""email"": ""efeo@geo.com"",
//      ""password"": ""oioioioi"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""ad58"",
//      ""username"": ""micke"",
//      ""email"": ""mikaelfant@gmail.com"",
//      ""password"": ""123"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""8224"",
//      ""username"": ""fefefef"",
//      ""email"": ""erik.cronqvist@gmail.com"",
//      ""password"": ""efe"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""0150"",
//      ""username"": ""micke"",
//      ""email"": ""mikaelfant@gmail.com"",
//      ""password"": ""123"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""d4fe"",
//      ""username"": ""micke2"",
//      ""email"": ""mikaelfant@gmail.com"",
//      ""password"": ""1212"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""b456"",
//      ""username"": ""brillinat_emma"",
//      ""email"": ""emma@trinket.us"",
//      ""password"": ""wow"",
//      ""role"": ""user""
//    },
//    {
//      ""id"": ""4dfc"",
//      ""username"": ""idiot"",
//      ""email"": ""apa@gmail.com"",
//      ""password"": ""q"",
//      ""role"": ""user""
//    }
//  ]");
app.MapGet("/ads", () => @"
[
    {
      ""id"": ""f1d9"",
      ""rubrik"": ""Frihet & fantasi"",
      ""län"": ""Halland"",
      ""boende"": ""Lägenhet"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""Mellanchef"",
      ""relstatus"": ""ensamvarg"",
      ""partnerinfo"": """",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjurHar"": true,
      ""husdjurHund"": false,
      ""husdjurKatt"": true,
      ""husdjurFågel"": false,
      ""husdjurHäst"": false,
      ""husdjurAnnat"": ""Gris"",
      ""stad"": ""Mellanstor stad"",
      ""stadAlternativ"": """",
      ""boendeSkog"": false,
      ""boendeHav"": false,
      ""boendeKultur"": false,
      ""boendeShopping"": true,
      ""bil"": true,
      ""bilinfo"": ""Ford Taunus"",
      ""barn"": false,
      ""fritidsintressen"": """",
      ""presentation"": ""Tjolahopp"",
      ""ålder"": ""18"",
      ""kön"": ""Man"",
      ""bids"": [
        ""8dde"",
        null,
        null,
        null,
        null,
        ""1"",
        ""e712"",
        ""1""
      ]
    },
    {
      ""id"": ""b4ed"",
      ""rubrik"": ""Enkelhet och lugn & ro på landet!"",
      ""län"": ""Skåne"",
      ""boende"": ""Husvagn"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""sjukpensionär"",
      ""relstatus"": ""ensamvarg"",
      ""partnerinfo"": """",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjurHar"": true,
      ""husdjurHund"": false,
      ""husdjurKatt"": false,
      ""husdjurFågel"": false,
      ""husdjurHäst"": false,
      ""husdjurAnnat"": ""två illrar"",
      ""stad"": ""Bystan"",
      ""stadAlternativ"": """",
      ""boendeSkog"": true,
      ""boendeHav"": true,
      ""boendeKultur"": false,
      ""boendeShopping"": false,
      ""bil"": false,
      ""bilinfo"": """",
      ""barn"": false,
      ""fritidsintressen"": ""fiskar, plockar bär och svamp, virkar, trippar"",
      ""presentation"": ""Tjoflöjt"",
      ""ålder"": ""54"",
      ""kön"": ""Kvinna"",
      ""bids"": [
        ""8dde""
      ]
    },
    {
      ""id"": ""a807"",
      ""rubrik"": ""Gay äldre lyx&kulturman"",
      ""län"": ""Stockholm"",
      ""boende"": ""Lägenhet"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""pensionär"",
      ""relstatus"": ""singel"",
      ""partnerinfo"": """",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjurHar"": false,
      ""husdjurHund"": false,
      ""husdjurKatt"": false,
      ""husdjurFågel"": false,
      ""husdjurHäst"": false,
      ""husdjurAnnat"": """",
      ""stad"": ""Storstad"",
      ""stadAlternativ"": ""Centralt"",
      ""boendeSkog"": false,
      ""boendeHav"": false,
      ""boendeKultur"": true,
      ""boendeShopping"": true,
      ""bil"": false,
      ""bilinfo"": """",
      ""barn"": false,
      ""fritidsintressen"": ""raggar unga män, går på opera, äter dyrt, promenerar på Djurgården"",
      ""presentation"": ""tjiipppi"",
      ""ålder"": ""77"",
      ""kön"": ""Man"",
      ""bids"": [
        ""8dde""
      ]
    },
    {
      ""id"": ""ee84"",
      ""rubrik"": ""Zombielycka"",
      ""län"": ""Blekinge"",
      ""boende"": ""Villa"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""Hemmafru"",
      ""relstatus"": ""exklusiv sambo"",
      ""partnerinfo"": ""44"",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjurHar"": false,
      ""husdjurHund"": false,
      ""husdjurKatt"": false,
      ""husdjurFågel"": false,
      ""husdjurHäst"": false,
      ""husdjurAnnat"": """",
      ""stad"": ""Småstad"",
      ""stadAlternativ"": """",
      ""boendeSkog"": true,
      ""boendeHav"": false,
      ""boendeKultur"": false,
      ""boendeShopping"": true,
      ""bil"": false,
      ""bilinfo"": """",
      ""barn"": false,
      ""fritidsintressen"": ""Sover, äter benso"",
      ""presentation"": ""flipflip"",
      ""ålder"": ""33"",
      ""kön"": ""Kvinna"",
      ""bids"": [
        ""8dde""
      ]
    },
    {
      ""id"": ""c07d"",
      ""rubrik"": ""This is it"",
      ""län"": ""Västerbotten"",
      ""boende"": ""Husvagn"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""brandman"",
      ""relstatus"": ""öppen särbo"",
      ""partnerinfo"": ""britta 48"",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjur"": """",
      ""Hund"": """",
      ""Katt"": """",
      ""Fågel"": """",
      ""Häst"": """",
      ""Annat"": """",
      ""stad"": ""Småstad"",
      ""stadAlternativ"": """",
      ""Skog"": """",
      ""Hav"": ""Hav / Sjö"",
      ""Kultur"": """",
      ""Shopping"": """",
      ""bil"": """",
      ""bilinfo"": """",
      ""barn"": """",
      ""fritidsintressen"": ""spelar poker"",
      ""presentation"": ""Do you want to eat ice cream?"",
      ""ålder"": ""37"",
      ""kön"": ""Kvinna"",
      ""publicerad"": true,
      ""enddate"": ""2024-04-30"",
      ""userId"": ""8224"",
      ""endTimestamp"": 1714435200000,
      ""bids"": [
        ""b456""
      ]
    },
    {
      ""id"": ""62e0"",
      ""rubrik"": ""Frihet & fantasi"",
      ""län"": ""Skåne"",
      ""boende"": ""Lägenhet"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""studerande"",
      ""relstatus"": ""exklusiv särbo"",
      ""partnerinfo"": """",
      ""barnAntal"": ""1"",
      ""barnBoende"": ""Delvis"",
      ""husdjur"": """",
      ""Hund"": """",
      ""Katt"": """",
      ""Fågel"": """",
      ""Häst"": """",
      ""Annat"": """",
      ""stad"": ""Storstad"",
      ""stadAlternativ"": """",
      ""Skog"": """",
      ""Hav"": ""Hav / Sjö"",
      ""Kultur"": ""Kultur"",
      ""Shopping"": ""Shopping"",
      ""bil"": ""bil"",
      ""bilinfo"": ""Mercedes"",
      ""barn"": ""barn"",
      ""fritidsintressen"": ""Spelar musik, lyssnar på musik, mm"",
      ""presentation"": ""Programmeringsstuderande 52-årig separerad  kulturman (utgiven romanförfattare, översättare) med särbegåvad varannanhelgsdotter och frispråkig, sexglad (kvinnlig) ingenjörssärbo, samt sällanrepande improvisationskrautband. Familjefritt boende i centrala Malmö. \n"",
      ""ålder"": ""52"",
      ""kön"": ""Man"",
      ""publicerad"": true,
      ""enddate"": ""2024-03-30"",
      ""userId"": ""d4fe"",
      ""endTimestamp"": 1711756800000,
      ""bids"": [
        ""b456""
      ]
    },
    {
      ""id"": ""6d36"",
      ""rubrik"": ""efw"",
      ""län"": """",
      ""boende"": """",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""sd"",
      ""relstatus"": """",
      ""partnerinfo"": """",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjur"": """",
      ""Hund"": """",
      ""Katt"": """",
      ""Fågel"": """",
      ""Häst"": """",
      ""Annat"": """",
      ""stad"": """",
      ""stadAlternativ"": """",
      ""Skog"": """",
      ""Hav"": """",
      ""Kultur"": """",
      ""Shopping"": """",
      ""bil"": """",
      ""bilinfo"": """",
      ""barn"": """",
      ""fritidsintressen"": ""wed"",
      ""presentation"": ""wd"",
      ""ålder"": """",
      ""kön"": """",
      ""publicerad"": false,
      ""enddate"": """",
      ""userId"": ""b456"",
      ""endTimestamp"": null
    },
    {
      ""id"": ""50e6"",
      ""rubrik"": ""vardagsromantism vid en sjö"",
      ""län"": ""Blekinge"",
      ""boende"": ""Husvagn"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""vävare"",
      ""relstatus"": ""knasigt"",
      ""partnerinfo"": """",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjur"": ""husdjur"",
      ""Hund"": """",
      ""Katt"": """",
      ""Fågel"": ""Fågel"",
      ""Häst"": """",
      ""Annat"": ""möss"",
      ""stad"": ""Bystan"",
      ""stadAlternativ"": """",
      ""Skog"": """",
      ""Hav"": ""Hav / Sjö"",
      ""Kultur"": ""Kultur"",
      ""Shopping"": """",
      ""bil"": """",
      ""bilinfo"": """",
      ""barn"": """",
      ""fritidsintressen"": ""drömmer och skriver dikter"",
      ""presentation"": ""jag är en modern version av lord byron. take it or leave it."",
      ""ålder"": ""22"",
      ""kön"": ""Man"",
      ""publicerad"": true,
      ""enddate"": ""2024-03-30"",
      ""userId"": ""b456"",
      ""endTimestamp"": 1711756800000,
      ""bids"": [
        ""b456""
      ]
    },
    {
      ""id"": ""b816"",
      ""rubrik"": ""djupa suckar och höga stämningar"",
      ""län"": ""Västerbotten"",
      ""boende"": ""Gård"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""gaffeltruckförare"",
      ""relstatus"": ""exklusiv särbo"",
      ""partnerinfo"": ""boel"",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjur"": ""husdjur"",
      ""Hund"": """",
      ""Katt"": """",
      ""Fågel"": """",
      ""Häst"": ""Häst"",
      ""Annat"": ""mullvad"",
      ""stad"": ""Småstad"",
      ""stadAlternativ"": """",
      ""Skog"": ""Skog"",
      ""Hav"": """",
      ""Kultur"": """",
      ""Shopping"": ""Shopping"",
      ""bil"": """",
      ""bilinfo"": """",
      ""barn"": """",
      ""fritidsintressen"": ""målar naglarna, röker pipa, knypplar "",
      ""presentation"": ""jag är en glad prick i sina bästa år. men jag saknar inte djup! nu är jag på jakt efter en själsfrände att loda avgrunden med."",
      ""ålder"": ""58"",
      ""kön"": ""Annat"",
      ""publicerad"": true,
      ""enddate"": ""2024-03-26"",
      ""userId"": ""b456"",
      ""endTimestamp"": 1711411200000
    },
    {
      ""id"": ""16fd"",
      ""rubrik"": ""fwd"",
      ""län"": ""Kalmar"",
      ""boende"": ""Gård"",
      ""boendeAnnat"": """",
      ""sysselsättning"": ""w4eradserfsed"",
      ""relstatus"": """",
      ""partnerinfo"": """",
      ""barnAntal"": """",
      ""barnBoende"": """",
      ""husdjur"": """",
      ""Hund"": """",
      ""Katt"": """",
      ""Fågel"": """",
      ""Häst"": """",
      ""Annat"": """",
      ""stad"": ""Mellanstor stad"",
      ""stadAlternativ"": """",
      ""Skog"": """",
      ""Hav"": ""Hav / Sjö"",
      ""Kultur"": """",
      ""Shopping"": """",
      ""bil"": """",
      ""bilinfo"": """",
      ""barn"": """",
      ""fritidsintressen"": ""gresfd"",
      ""presentation"": ""wfde"",
      ""ålder"": ""20"",
      ""kön"": ""Annat"",
      ""publicerad"": true,
      ""enddate"": ""2024-04-25"",
      ""userId"": ""b456"",
      ""endTimestamp"": 1714003200000,
      ""bids"": [
        ""b456""
      ]
    }
  ]"
  );



app.Run("http://localhost:3001");

public class LoginRequest
{
    public string Username { get; set; }
    public string Password { get; set; }
}
